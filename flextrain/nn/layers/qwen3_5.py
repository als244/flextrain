...
class Qwen3_5LinearLayer:
    ...
    def forward_recompute(
        ...
        if need_fla:
            # Cross-chunk conv state (Item 3c, C8): use ``_fwd_conv`` so
            # the recompute reads ``ctx.lin_conv_fwd_window`` as
            # initial_state for continuation chunks. ``recompute_only``
            # idempotently re-writes slot.lin_conv_state but does NOT
            # advance the global window.
